...
class MPLight(SharedAgent):
    def __init__(self, config, obs_act, map_name, thread_number, load_model, test=False):
        super().__init__(config, obs_act, map_name, thread_number, load_model, test)
        ...
        if load_model:
            model = self.load_model(load_model)
        else:
    ...
    def save_model(self, path: str):
        with open(path, 'wb') as f:
            pickle.dump(self.agent.model, f)
...
class FRAP(nn.Module):
    ...
    def forward(self, states):
        ...
        for pair in self.phase_pairs:
            pairs.append(sum(phase_demands[:, pair[n]] for n in range(len(pair))))
            # if len(pair) == 1:
            #     pairs.append(phase_demands[:, pair[0]])
            # else:
