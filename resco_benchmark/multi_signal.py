...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, net, state_fn, reward_fn, route=None, gui=False, end_time=3600,
                 ...
                 warmup=0, gymma=False, output=True, fast=False):
        ...
        if self.output is True:
            sumo_cmd.extend(["--statistic-output",
            "travel_stats.xml",
            "--duration-log.statistics"])
        if self.fast is True:
            sumo_cmd.extend(["--max-depart-delay", "500"])
    ...
    def calc_metrics(self, rewards):
        ...
        for signal_id in self.signals:
            ...
            for lane in signal.lanes:
                lane_veh = signal.full_observation[lane]['vehicles']
                if lane_veh:
                    for key, value in lane_veh.items():
    ...
    def save_metrics(self):
        ...
        veh_log = os.path.join(self.log_dir, self.connection_name+ os.sep + 'vehicles_' + str(self.run) + '.json')
        print('saving vehicles', veh_log)
        with open(veh_log, 'w+') as output_veh:
            json.dump(self.vehicles, output_veh)

    def save_unfinished(self):  # only call before closing the simulation
        unfinished = {}
        vehicles = self.sumo.vehicle.getIDList()
        for veh in vehicles:
            distance = self.sumo.vehicle.getDistance(veh)
            unfinished[veh] = distance
        unfinished_log = os.path.join(self.log_dir, self.connection_name + os.sep + 'unfinished_' + str(self.run) + '.json')
        with open(unfinished_log, 'w+') as output_unfinished:
            json.dump(unfinished, output_unfinished)
