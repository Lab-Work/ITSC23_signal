...
def main():
    ...
    ap.add_argument("--agent", type=str, default='MAXPRESSURE',
    ...
    ap.add_argument("--map", type=str, default='ingolstadt7x',
                    choices=['grid4x4', 'arterial4x4', 'ingolstadt1', 'ingolstadt7', 'ingolstadt7x',
                             'ingolstadt7low', 'ingolstadt7mid', 'ingolstadt7hig', 'ingolstadt21',
    ...
    ap.add_argument("--tr", type=int, default=5)  # Can't multi-thread with libsumo, provide a trial number
    ap.add_argument("--output", type=bool, default=False)
    ap.add_argument("--fast", type=bool, default=True)
...
def run_trial(args, trial):
    ...
    env = MultiSignal(alg.__name__+'-tr'+str(trial),
                      ...
                      step_ratio=map_config['step_ratio'], end_time=map_config['end_time'], fast=args.fast,
                      max_distance=agt_config['max_distance'], lights=map_config['lights'], gui=args.gui,
                      log_dir=args.log_dir, libsumo=args.libsumo, output=args.output, warmup=map_config['warmup'])

    agt_config['episodes'] = int(args.eps * 0.9)    # schedulers decay over 80% of steps
    ...
    agent = alg(agt_config, obs_act, args.map, trial, args.load_model)

    for eps in tqdm(range(args.eps)):
        obs = env.reset()
        done = False
        while not done:
            act = agent.act(obs)
...
if __name__ == '__main__':
    # algs = ["MAXPRESSURE", "MAXWAVE", "MPLight"]
    # eps = 25
    #
    # maps = ['ingolstadt7mid', 'ingolstadt7hig', 'ingolstadt7plus']
    # load_models = [None]
    # for agent, load_model in zip(algs, load_models):
    #     for map in maps:
    #         sys.argv = ['']  # this is the script name (it can be anything)
    #         sys.argv.append(f"--eps={eps}")
    #         sys.argv.append(f"--agent={agent}")
    #         sys.argv.append(f"--load_model={load_model}")
    #         sys.argv.append(f"--map={map}")
    #         sys.argv.append(f"--test={True}")
    #         main()
